    ...
    if (slider) {
        ...
        const slide = (list, action) => {
            ...
            if (action === 'next') {
                ...
                if (list[currentSlide - 1]) {
                    list[currentSlide + 1].classList.add('slider-item__down');
                    list[currentSlide].classList.remove('slider-item__active');
                    list[currentSlide - 1].classList.add('slider-item__active');
                    btnPrev.classList.add('orange-left');
                    btnNext.classList.remove('orange-right');
                    slider.querySelector('.slider-item').
                }
            }
        }
        ...
        const maxSliderHeight = (list) => {
            return Array.from(list).reduce((item, nextItem) => { return item.offsetHeight > nextItem.offsetHeight ? item.offsetHeight : nextItem.offsetHeight; })
        }

        slider.querySelector('.slider-wrapper').style.height = maxSliderHeight(sliderList) + 30 + 'px';
